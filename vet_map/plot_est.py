...
# ── Carga y filtrado ─────────────────────────────────────────────────────────
df = pd.read_csv("vet_map/denue_inegi_81_1.csv", encoding="latin1", low_memory=False)
...
mask = (df["cve_ent"] == 14) & (df["cve_mun"].isin(ZMG)) & (df["codigo_act"] == 812990)
df = df[mask].copy()
...
print(f"Establecimientos SCIAN 812990 en ZMG: {len(df)}")
...
<div style="
    ...
    max-width:270px">
  <b style="font-size:14px">SCIAN 812990</b><br>
